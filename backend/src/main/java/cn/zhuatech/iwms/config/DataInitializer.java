...
package cn.zhuatech.iwms.config;
import cn.zhuatech.iwms.domain.DomainCatalog;
import cn.zhuatech.iwms.model.*;
import cn.zhuatech.iwms.repository.*;
...
@Configuration public class DataInitializer {
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "IWMS-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="IWMS-DEMO-"+String.format("%03d",sequence);
