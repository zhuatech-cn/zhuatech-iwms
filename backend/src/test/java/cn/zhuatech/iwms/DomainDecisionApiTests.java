...
@SpringBootTest @AutoConfigureMockMvc class DomainDecisionApiTests {
 ...
 @Test void domainDecisionReturnsAuditableScoreMetricsAndActions() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"workOrderNo\":\"IWMS-WO-2026-88\",\"priority\":\"紧急\",\"elapsedHours\":3,\"slaHours\":4,\"technicianAssigned\":true,\"partsAvailable\":true,\"safetyPermitRequired\":true,\"safetyPermitApproved\":true}"))
 ...
 @Test void domainRiskScenarioReturnsExpectedBlockingDecision() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"workOrderNo\":\"IWMS-WO-2026-88\",\"priority\":\"紧急\",\"elapsedHours\":10,\"slaHours\":2,\"technicianAssigned\":false,\"partsAvailable\":false,\"safetyPermitRequired\":true,\"safetyPermitApproved\":false}"))
